    ...
    <>
      <style dangerouslySetInnerHTML={{ __html: `
        .chatknopf{position:fixed;right:22px;bottom:22px;z-index:200;width:58px;height:58px;border-radius:50%;
          background:${D.blau};color:#fff;border:none;cursor:pointer;font-size:21px;
        ...
        .chatkopf{background:${D.dunkel};color:#fff;padding:17px 18px;display:flex;align-items:center;gap:12px}
        .chatzeile{display:flex;align-items:center;gap:12px;padding:13px 16px;border-bottom:1px solid ${D.linie};
          font-size:14px;font-weight:600;color:#41506B;transition:background .14s,padding-left .14s}
        .chatzeile:hover{background:${D.blauZart};color:${D.blau};padding-left:20px}
        ...
        .chatfeld{width:100%;border:2px solid ${D.linie};border-radius:10px;padding:11px 13px;font-size:14px;
          outline:none;resize:none;transition:border-color .16s}
      ...
      {offen && (
        <div className="chatfenster" role="dialog" aria-label="Hilfe und Kontakt">
          <div className="chatkopf">
            <div style={{ width: 38, height: 38, borderRadius: '50%', background: D.blau, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
              <i className="fa-solid fa-headset" aria-hidden="true" />
            </div>
            <div style={{ flex: 1 }}>
              <div style={{ fontSize: 14.5, fontWeight: 800 }}>Wir helfen gern</div>
              <div style={{ fontSize: 12, color: '#9FB0C8' }}>Mo. – Fr., 9 – 18 Uhr</div>
            </div>
            <button onClick={() => setOffen(false)} aria-label="Schließen"
              style={{ background: 'none', border: 'none', color: '#9FB0C8', cursor: 'pointer', fontSize: 17, padding: 4 }}>
