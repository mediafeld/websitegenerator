    ...
    <>
      <style dangerouslySetInnerHTML={{ __html: `
        .chatknopf{position:fixed;right:22px;bottom:22px;z-index:200;width:58px;height:58px;border-radius:50%;
          background:${D.akzent};color:#fff;border:none;cursor:pointer;font-size:21px;
          box-shadow:0 10px 28px rgba(232,54,93,.4);transition:transform .2s cubic-bezier(.2,.7,.3,1),background .18s}
        .chatknopf:hover{transform:translateY(-3px) scale(1.04);background:${D.akzentHell}}
      ...
      {offen && (
        <div className="chatfenster" role="dialog" aria-label="Hilfe und Kontakt">
          <div className="chatkopf">
            <div style={{ width: 38, height: 38, borderRadius: '50%', background: D.akzent, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
