...
export function Seite({ eyebrow, titel, titelLeicht, einleitung, children, css = '' }) {
  return (
    <div style={{ background: D.dunkel, color: D.text, fontFamily: '"Inter Tight",system-ui,sans-serif', minHeight: '100vh', display: 'flex', flexDirection: 'column' }}>
      ...
      <section style={{ background: D.dunkel, color: '#fff', padding: '54px 0 50px', position: 'relative', overflow: 'hidden' }}>
        <div aria-hidden="true" style={{ position: 'absolute', inset: 0, background: `radial-gradient(760px 340px at 18% -20%, ${D.magenta}44, transparent 68%)` }} />
        <div className="wrap" style={{ position: 'relative', maxWidth: 860 }}>
          <a href="/" className="link-u" style={{ fontSize: 13, color: D.grauHell, display: 'inline-block', marginBottom: 18 }}>← Zurück zur Startseite</a>
          {eyebrow && <p className="eyebrow" style={{ color: D.tuerkis, marginBottom: 14 }}>{eyebrow}</p>}
          <h1 className="display" style={{ fontSize: 'clamp(30px,5vw,50px)', marginBottom: 16 }}>
            {titel} {titelLeicht && <span style={{ fontWeight: 300, color: D.textMatt }}>{titelLeicht}</span>}
          </h1>
          {einleitung && <p style={{ fontSize: 16.5, color: D.textMatt, maxWidth: 640, lineHeight: 1.68 }}>{einleitung}</p>}
        </div>
      </section>
...
export function Abschluss({ titel = 'Schauen kostet nichts.', text = 'Geh die Fragen durch und sieh dir das Ergebnis an. Bezahlt wird erst, wenn dir die Website gefällt.' }) {
  return (
    <section style={{ padding: '20px 0 76px' }}>
      <div className="wrap">
        <div style={{ background: 'linear-gradient(140deg,#2B0B5A,#5B1090 48%,#0B3C4A)', borderRadius: 18, padding: '48px 34px', textAlign: 'center' }}>
          <h2 className="display" style={{ fontSize: 'clamp(24px,3.4vw,34px)', color: '#fff', marginBottom: 12 }}>{titel}</h2>
          <p style={{ fontSize: 15.5, color: '#E4D8FF', maxWidth: 480, margin: '0 auto 26px', lineHeight: 1.65 }}>{text}</p>
