...
.sblock{background:${D.weiss};border:1px solid ${D.linie};border-radius:14px;overflow:hidden;margin-bottom:14px}
.skopf{font-size:10.5px;font-weight:700;letter-spacing:.14em;text-transform:uppercase;color:${D.grauHell};padding:14px 16px 9px}
.spunkt{display:flex;align-items:center;gap:11px;padding:11px 16px;font-size:13.8px;font-weight:600;color:#41506B;border-left:3px solid transparent;transition:all .16s}
.spunkt:hover{background:${D.blauZart};color:${D.blau};border-left-color:${D.blau};padding-left:20px}
.spunkt i{width:16px;text-align:center;font-size:13px;color:${D.grauHell};transition:color .16s,transform .16s}
...
.kkarte{background:${D.weiss};border:1px solid ${D.linie};border-radius:16px;padding:30px 30px}
.kkarte h2{font-size:21px;font-weight:800;letter-spacing:-.025em;margin-bottom:6px}
.kkarte .unter{font-size:14px;color:${D.grau};margin-bottom:22px;line-height:1.6}
.feld{width:100%;padding:13px 15px;font-size:15px;border:2px solid ${D.linie};border-radius:10px;outline:none;transition:border-color .16s,box-shadow .16s;background:#fff}
.feld:focus{border-color:${D.blau};box-shadow:0 0 0 4px ${D.blau}18}
.beschr{display:block;font-size:12.5px;font-weight:700;color:#41506B;margin-bottom:7px}
  ...
  if (!nutzer) return rahmen(
    <Mitte>
      <div style={{ textAlign: 'center' }}>
        <p style={{ fontSize: 15, color: D.grau, marginBottom: 16 }}>Bitte melde dich an, um diesen Bereich zu sehen.</p>
      ...
      <div className="wrap kgrid" style={{ maxWidth: 1160 }}>
        ...
        <aside className="sidebar">
          <div className="sblock">
            <div style={{ padding: '16px 16px 14px', borderBottom: `1px solid ${D.linie}`, display: 'flex', alignItems: 'center', gap: 11 }}>
              ...
              <div style={{ minWidth: 0 }}>
                ...
                <div style={{ fontSize: 11.5, color: D.grauHell, overflow: 'hidden', textOverflow: 'ellipsis' }}>{nutzer.email}</div>
              </div>
            </div>
            {BEREICHE.map(b => (
              <div key={b.gruppe}>
          ...
          <div className="sblock" style={{ padding: 16 }}>
            ...
            <a className="link-u" href={TELEFON_LINK} style={{ fontSize: 13.5, fontWeight: 700, color: D.dunkel, display: 'block', marginBottom: 4 }}>
              <i className="fa-solid fa-phone" style={{ marginRight: 8, color: D.blau }} aria-hidden="true" />{TELEFON}
            </a>
            <a className="link-u" href={`mailto:${EMAIL}`} style={{ fontSize: 13, color: D.grau, display: 'block' }}>
              <i className="fa-solid fa-envelope" style={{ marginRight: 8, color: D.blau }} aria-hidden="true" />{EMAIL}
            </a>
            <p style={{ fontSize: 12, color: D.grauHell, marginTop: 10 }}>Mo. – Fr., 9 – 18 Uhr</p>
          </div>
        </aside>
        ...
        <main>
          ...
          {unter && <p style={{ fontSize: 15, color: D.grau, marginBottom: 24, lineHeight: 1.6, maxWidth: 640 }}>{unter}</p>}
