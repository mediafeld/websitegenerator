...
export default function PasswortNeu() {
  ...
  return (
    <div style={{ minHeight: '100vh', background: D.paper, color: D.dunkel, fontFamily: '"Inter Tight",system-ui,sans-serif', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 22 }}>
      <link href="/schrift/schrift.css" rel="stylesheet" />
      <link href="/fa/css/all.min.css" rel="stylesheet" />
      <style dangerouslySetInnerHTML={{ __html: BASIS_CSS + `
        .lfeld{width:100%;padding:14px 16px 14px 44px;font-size:15px;border:2px solid ${D.linie};border-radius:11px;outline:none;background:#fff;transition:border-color .16s}
      ...
      <div style={{ width: '100%', maxWidth: 430, background: '#fff', border: `1px solid ${D.linie}`, borderRadius: 18, padding: '38px 36px', boxShadow: '0 14px 44px rgba(10,24,36,.08)' }}>
        <div style={{ textAlign: 'center', marginBottom: 24 }}>
          <a href="/" className="display" style={{ fontSize: 20 }}>websitegenerator<span style={{ color: D.blau }}>24</span></a>
        ...
        <p style={{ fontSize: 12.5, color: D.grauHell, marginTop: 20, textAlign: 'center', lineHeight: 1.7 }}>
          Probleme? <a className="link-u" href={`mailto:${EMAIL}`} style={{ color: D.blau }}>{EMAIL}</a>
        </p>
      </div>
    </div>
  )
